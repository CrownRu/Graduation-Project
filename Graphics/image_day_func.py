...
###############################
#該檔案為日圖表只會進入以下37行的區塊，圖表是以往前推一天中的所有資料的時間(幾點)做區分
###############################
    if n == 1:
        if specific_date in combined_map:
            times, scores = zip(*combined_map[specific_date])
            plt.plot(times, scores, marker='o', linestyle='-', linewidth=7, color='#8b4513')
            # plt.xlabel('Time')
            # plt.ylabel('Score')
        ...
        if daily_average_scores:
            ...
            plt.axhline(y=0.60, color='green', linestyle='--')
...
###############################
#image_day_function為主要運作的主要function，將圖表生成的圖片生成並上傳到主機
###############################
def image_day_function(user_number):
    ...
    try:
        ...
        rslt_plot = draw_combined_map_plot(*map_process_rslt, current_date, 1)
        imagefilename="user"+str(user_number)+"_"+str(current_date)+"_day.png"
        ...
        cursor.execute(sql,(imagefilename,user_number,"day",current_date))
        connection.commit()
