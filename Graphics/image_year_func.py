...
###############################
#該檔案為週圖表只會進入以下48行的區塊，圖表是以當日往前推365天當作來源依據
###############################
    elif n > 1:
        ...
        # 將每日分數按月分組並計算每月平均分數
        monthly_average_scores = defaultdict(list)
        for date, scores in daily_average_scores.items():
            year_month_key = datetime.strptime(date, "%Y-%m-%d").strftime('%Y-%m')
            monthly_average_scores[year_month_key].extend(scores)

        # 計算每月平均分數
        average_scores = [sum(scores) / len(scores) for scores in monthly_average_scores.values()]
        months = list(monthly_average_scores.keys())

        plt.plot(months, average_scores, linestyle='-', linewidth=7, color='#8b4513')
        ...
        #     print(dates)
        #     data_cut_num=len(dates)//8
        #     if data_cut_num==0:
        #         data_cut_num=1

        #     #print("data_cut_num:"+str(data_cut_num))
...
###############################
#image_year_function為主要運作的主要function，將圖表生成的圖片生成並上傳到主機
###############################
def image_year_function(user_number):
    ...
    try:
        ...
        rslt_plot = draw_combined_map_plot(*map_process_rslt, current_date, 365)
        imagefilename="user"+str(user_number)+"_"+str(current_date)+"_year.png"
        ...
        cursor.execute(sql,(imagefilename,user_number,"year",current_date))
        connection.commit()
