...
###############################
#該檔案為週圖表只會進入以下51行的區塊，圖表是以當日往前推7天當作來源依據
###############################
    elif n > 1:
...
###############################
#image_week_function為主要運作的主要function，將圖表生成的圖片生成並上傳到主機
###############################
def image_week_function(user_number):
    ...
    try:
        ...
        rslt_plot = draw_combined_map_plot(*map_process_rslt, current_date, 7)
        imagefilename="user"+str(user_number)+"_"+str(current_date)+"_week.png"
        ...
        cursor.execute(sql,(imagefilename,user_number,"week",current_date))
        connection.commit()
